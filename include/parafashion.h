...
public:
    ...
    PatchGraph<TraceMesh>* pagraph = nullptr;
    std::vector<Segment<TriMeshType>*> segmentps;
    ...
    void TracePatch(bool SaveStep=true,
                    bool DebugMSG=false)//MYNOTE: the core of generating patch layout.
    {
        if (SaveStep)
        {
            deformed_mesh.Clear();
        ...
        typedef PatchTracer<TriMeshType,MeshArapQuality<TriMeshType> > PTracerType; // MYNOTE: keep vector field, work on node graph to generate seam.
        ...
        // MYNOTE: above is the settings for path tracer.
    ...
    void BuildPatchGraph(){
        Parametrizer<TriMeshType>::MergeAcrossBoundarySeams(deformed_mesh);
        ...
        RetrievePatchesFromSelEdges<TriMeshType> (deformed_mesh, StartF, Partitions);
        PatchManager<TriMeshType>::DerivePerFacePartition(deformed_mesh, Partitions, FacePartitions);
        pagraph = new PatchGraph<TriMeshType> (deformed_mesh, Partitions, FacePartitions); 

        pagraph->InitPatches();
        std::cout<<"-----------------------------"<<std::endl;
        std::cout<<"Num Partitions: "<<Partitions.size()<<std::endl;
        std::cout<<"Num Sub meshes: "<<SubMeshes.size()<<std::endl;//0
        std::cout<<"Num Patches: "<<pagraph->num_patch()<<std::endl;
        pagraph->InitConnections();
        std::cout<<"Num Edges: "<<pagraph->seamEdges.size()<<std::endl;
        std::cout<<"-----------------------------"<<std::endl;
    }

    void BuildSegments(const std::vector<std::vector<int>> *manualPatchSets = nullptr){
        // createSegments<TriMeshType>(*pagraph, segmentps);
        createSegmentsfromSids<TriMeshType>(*pagraph, segmentps, 2, 2, true, 300, manualPatchSets);
        std::cout<<"Built segments"<<std::endl;
        for (auto segp:segmentps)
        {
            segp->build_approxPolygon();
        }
        std::cout<<"Built Polygons Done."<<std::endl;
    ...
    void BatchProcess(const std::vector<std::vector<CoordType> > &PickedPoints,
                      ...
                      bool writeTime=true) // MYNOTE: the core running logic.
    {
        RestoreInitMesh();
        size_t t0=clock();
...
//        }
        ...
        #endif
        //Debug
        std::cout<<"Total Area 2:"<<std::endl;
        std::cout<<vcg::tri::Stat<TraceMesh>::ComputeMeshArea(deformed_mesh)<<std::endl;
        std::cout<<vcg::tri::Stat<TraceMesh>::ComputeMeshArea(reference_mesh)<<std::endl;
        //End Debug
        BuildPatchGraph();
        BuildSegments();
